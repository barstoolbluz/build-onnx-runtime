# ONNX Runtime 1.20.1 for NVIDIA Hopper (SM90: H100, L40S) + AVX2
...
let
  ...
  gpuArchCMake = "90";
  cpuFlags = [ "-mavx2" "-mfma" ];
  variantName = "onnxruntime-python313-cuda12_9-sm90-avx2";
  # ────────────────────────────────────────────────────────────────────
...
in
  (nixpkgs_pinned.python3Packages.onnxruntime.override {
    ...
    pname = variantName;
    meta = oldAttrs.meta // {
      description = "ONNX Runtime 1.20.1 for NVIDIA H100/L40S (SM90) + AVX2 [CUDA 12.9]";
