# ONNX Runtime 1.20.1 for NVIDIA Ampere DC (SM80: A100, A30) + AVX2
...
let
  ...
  gpuArchCMake = "80";
  cpuFlags = [ "-mavx2" "-mfma" ];
  variantName = "onnxruntime-python313-cuda12_9-sm80-avx2";
  # ────────────────────────────────────────────────────────────────────
...
in
  (nixpkgs_pinned.python3Packages.onnxruntime.override {
    ...
    pname = variantName;
    meta = oldAttrs.meta // {
      description = "ONNX Runtime 1.20.1 for NVIDIA A100/A30 (SM80) + AVX2 [CUDA 12.9]";
