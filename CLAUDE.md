...
- ONNX Runtime base: 1.23.2, overridden to **1.24.2** via `version`/`src`/`patches`/`postPatch`/`cmakeFlags` override
- CUTLASS: 4.2.1 (overridden via `FETCHCONTENT_SOURCE_DIR_CUTLASS`, GPU variants only)
- ONNX: 1.20.1 (overridden via `FETCHCONTENT_SOURCE_DIR_ONNX`, all variants)
...
- **ort-1.24**: ONNX Runtime 1.24.2 + CUDA 12.9, driver 560+ (current)
- Future branches: `ort-1.22`, etc. for other ORT versions

### CUDA Version Documentation
Each GPU `.nix` file includes a two-line header comment:
```nix
# ONNX Runtime 1.24.2 for NVIDIA Hopper (SM90: H100/L40S) + AVX-512
...
- Provide compatible `postPatch` for the new source
