...
CUDA 12.4 variants use Python 3.12 (`python312Packages.onnxruntime`); CUDA 12.9 and CPU-only variants use Python 3.13 (`python3Packages.onnxruntime`). The CUDA minor version and Python version are encoded in the filename so the exact toolkit is always visible.
...
description = "ONNX Runtime 1.22.2 for NVIDIA H100/L40S (SM90) + AVX-512 [CUDA 12.4]";
```
